...
class AccountMovementServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        accountMovementDto = new AccountMovementDto(1, "1234", OriginMovement.DOMICILIACION, new Date(), 100.0, MovementType.DEPOSITAR, "Test Deposit");
        accountMovementEntity = new AccountMovementEntity(1, "1234", OriginMovement.DOMICILIACION, new Date(), 100.0, MovementType.DEPOSITAR, "Test Deposit");
        
        // IMPORTANT: The service uses AccountMovementMapper which is a singleton that works with static instance.
        // In unit tests, we usually rely on the real mapper if it's stateless or well-behaved, 
        // OR we can't easily mock it unless we mock the static getInstance or use setter injection if available.
        // Looking at AccountMovementMapper, it's a singleton but has no external dependencies for mapping entity<->dto except creating new objects.
        // So we can use the real mapper indirectly by mocking the repository to return Entity and asserting DTO.
    }
    ...
    class FindAllTests {
        ...
        void shouldReturnAllAccountMovements() {
            ...
            // Since we use real mapper (implicitly), we check if data matches
            assertEquals(accountMovementEntity.idAccountMovent(), result.get(0).idAccountMovement());
            verify(accountMovementRepository, times(2)).findAllAccountMovements(); // Called once for isEmpty check, once for stream
        }
        ...
        void shouldThrowExceptionWhenNoMovements() {
            ...
            RuntimeException exception = assertThrows(RuntimeException.class, () -> accountMovementService.findAllAccountMovements());
    ...
    class FindByIdTests {
        ...
        void shouldThrowExceptionWhenGetNotFound() {
            ...
            RuntimeException exception = assertThrows(RuntimeException.class, () -> accountMovementService.getAccountMovementById(1));
